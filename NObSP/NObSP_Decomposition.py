...
def NObSP_NN_single(X, y_est, model):
    ...
    y_est = y_est.reshape((-1,1)) # Converting to a vector with dimension 1
    P_xy = np.zeros((N,N,d)) # Initializing proyection matrices
    y_e = torch.from_numpy(np.zeros((N,d))).type(torch.float) # Initializing Matriz where the estimated nonlinear contributions will be stored, converting to a tensor object
    ...
    Alpha = torch.zeros(neurons_last,d) # Initializing the matrix for the Alpha coefficients, out-of-sample extension, using the dimension where the transformed data lies
    ...
    for i in range(d):
        ...
        with torch.inference_mode():
            y_target, X_target_sub = model(X_target) # X_target_sub is a basis for the nonlienar transformation of the data in X_target
            y_reference, X_reference_sub = model(X_reference) # X_reference_sub is a basis for the nonlienar transformation of the data in X_reference
        ...
        y_e[:,[i]] = P@(y_est-y_est.mean()) # Using the projection matrices to ptoject the output vector and find the nonlinear contribution of each variable.
...
def NObSP_NN_2order(X, y_est, P1, model):
    ...
    d = np.size(X,1) # computing the size of X along dimension 1
    d_com = np.int_(d*(d-1)/2) # Number of interaction effects to compute
    P1 = torch.from_numpy(P1).type(torch.float) # Converting the tensor with projection matrices to a pytorch tensor
    y_est = y_est.reshape((-1,1)) # Converting to a vector with dimension 1
    P_xy = np.zeros((N,N,d_com)) # Initializing proyection matrices
    y_e = torch.from_numpy(np.zeros((N,d_com))).type(torch.float) # Initializing Matriz where the estimated nonlinear contributions will be stored
    ...
    for i in range(d):
        for k in range(i+1,d):
            ...
            # Computing the transformation of the input data using inference mode in order to find the basis for the susbpace of the transformations
            with torch.inference_mode():
                y_target, X_target_sub = model(X_target) # X_target_sub is a basis for the nonlinear transformation of the data in X_target
                y_reference, X_reference_sub = model(X_reference) # X_reference_sub is a basis for the nonlienar transformation of the data in X_reference
            ...
            P = torch.from_numpy(P_xy[:,:,index_aux]).type(torch.float) # Converting form pytorch to numpy
            y_e[:,[index_aux]] = P@(y_est-y_est.mean()) # Using the projection matrices to ptoject the output vector and find the nonlinear contribution of each variable.
