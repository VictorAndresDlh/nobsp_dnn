...
print(df.head())
X = df.drop(['csMPa'],axis=1).to_numpy(dtype='float')
y = df['csMPa'].to_numpy(dtype='float')

# Initializing variables
epochs = 1000
learning_rate = 0.005 # DEfining learning rate of the model
N = len(y)
t = np.arange(0,N)
train_split = int(0.8*N) # 80% of the data to be used as training and compute projections
...
axs[2,1].scatter(y.cpu(),y_est_1.cpu(), marker='x') # Calinbration plot real output vs estiamted output
plt.show()
...
axs[0].plot(t, y, t, y_est_1.cpu()) # Estimated output of the complete model vs real output
axs[1].scatter(y, y_est_1.cpu(), marker='x') # Calibration plot between estimated output of the complete model vs real output
